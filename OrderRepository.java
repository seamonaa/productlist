...
public class OrderRepository {
    private static final String DATA_FILE = "product.txt";

   
    public static void saveOrder(SkincareItem[] items, int count, double totalCost) throws IOException {
        BufferedWriter bw = new BufferedWriter(new FileWriter(DATA_FILE, true));

        for (int i = 0; i < count; i++) {
            bw.write(items[i].display());
            bw.newLine();
        }
